...
<!-- ************************************************************** -->
<!--
NAME:     File      = 2mup.xsl (version 1.0.3)
          Vers Date = 2010/05/24
...
<xsl:stylesheet version="2.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
  ...
  <xsl:variable name="xslversion">
    <xsl:text>2mup.xsl (version 1.0.3)</xsl:text>
  </xsl:variable>
  ...
  <xsl:template match="mei:meihead">
    ...
    <xsl:for-each select="mei:encodingdesc[mei:projectdesc[descendant::text()]]">
      ...
      <xsl:text>// Converted to Mup using 2mup.xsl </xsl:text>
