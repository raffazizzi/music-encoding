...
<xsl:stylesheet version="2.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
  ...
  <xsl:output method="xml" indent="yes" encoding="UTF-8" omit-xml-declaration="no"
    standalone="no"
    doctype-system="file://C:\Documents and Settings\pdr4h\Desktop\SVN-MEI\branches\mei19\DTD\mei19-all-test.dtd"
              ...
              <xsl:for-each select="midi-instrument">
                <instrdef>
                  ...
                  <xsl:attribute name="midi.instrname">
                    <xsl:value-of
                      select="preceding-sibling::score-instrument[@id=$midiID]"/>
                  </xsl:attribute>
                </instrdef>
              </xsl:for-each>
            </xsl:when>
        ...
        <staffgrp>
          ...
          <xsl:for-each select="midi-instrument">
            <instrdef>
              ...
              <xsl:attribute name="midi.instrname">
                <xsl:value-of select="preceding-sibling::score-instrument[@id=$midiID]"/>
              </xsl:attribute>
            </instrdef>
          </xsl:for-each>
            ...
            <xsl:when
              ...
              <rend>
                <xsl:if test="metronome/@font-family">
                  <xsl:attribute name="fontfam">
                    <xsl:value-of select="normalize-space(metronome/@font-family)"/>
                  </xsl:attribute>
                </xsl:if>
  ...
  <xsl:template match="note/notations/tuplet[@type='start']" mode="stage1">
    ...
    <tupletspan>
      ...
      <xsl:for-each select="ancestor::note[1]/time-modification">
        <xsl:attribute name="num"><xsl:value-of select="actual-notes"/>:<xsl:value-of
            select="normal-notes"/></xsl:attribute>
        ...
        <note>
          ...
          <xsl:choose>
            <xsl:when test="stem='up'">
              ...
              <xsl:if test="stem/@default-y">
                <xsl:attribute name="stem.y">
                  <xsl:value-of select="abs(round-half-to-even(stem/@default-y div 10,3))"
                  />
                </xsl:attribute>
              </xsl:if>
            </xsl:when>
            <xsl:when test="stem='down'">
              ...
              <xsl:if test="stem/@default-y">
                <xsl:attribute name="stem.y">
                  <xsl:value-of select="abs(round-half-to-even(stem/@default-y,3))"/>
                </xsl:attribute>
              </xsl:if>
            </xsl:when>
  ...
  <xsl:template name="part-layer-staff-beam-assign">
    ...
    <xsl:choose>
      <xsl:when test="beam[@number='1']='begin'">
        ...
        <xsl:attribute name="beam">i<xsl:value-of select="$beamlevel"/></xsl:attribute>
      </xsl:when>
      ...
      <xsl:when test="beam[@number='1']='end'">
        ...
        <xsl:attribute name="beam">t<xsl:value-of select="$beamlevel"/></xsl:attribute>
      </xsl:when>
      <xsl:when
...
sum(preceding-sibling::forward/duration) - sum(preceding-sibling::backup/duration)"
  ...
  <xsl:template name="getstaffnum">
    ...
    <xsl:variable name="ingroup">
      <xsl:value-of
        select="$defaultlayout//staffgrp[@id=$partID]/staffdef[position()=$partstaff]/@n"
      />
    </xsl:variable>
    <xsl:variable name="indef">
      <xsl:value-of select="$defaultlayout//staffdef[@id=$partID]/@n"/>
    </xsl:variable>
    <xsl:choose>
  ...
  <xsl:template match="body/mdiv/score/scoredef[1]/staffgrp" priority="2" mode="stage2">
    <staffgrp>
      <xsl:copy-of select="@*[not(name()='meiform')]"/>
      <xsl:copy-of select="instrdef"/>
                    ...
                    <xsl:when test="count(ancestor::staffgrp[1]/instrdef) &gt; 1">
                      <xsl:variable name="layers">
                        ...
                        <xsl:for-each select="ancestor::staffgrp[1]/instrdef">
                          <xsl:variable name="thisinstr" select="@id"/>
                          <xsl:if
  ...
  <xsl:template match="tupletspan" priority="2" mode="stage2">
    <tupletspan>
      <xsl:copy-of select="@*[not(name()='meiform' or name()='start' or name()='end')]"/>
      <xsl:variable name="start">
        <xsl:value-of select="@start"/>
      </xsl:variable>
      <xsl:choose>
        <xsl:when test="preceding::note[@id=$start]/parent::chord">
          <xsl:attribute name="start">
            <xsl:value-of select="preceding::note[@id=$start]/parent::chord/@id"/>
          </xsl:attribute>
        </xsl:when>
        <xsl:otherwise>
      ...
      <xsl:choose>
        <xsl:when test="preceding::note[@id=$end]/parent::chord">
          <xsl:attribute name="end">
            <xsl:value-of select="preceding::note[@id=$end]/parent::chord/@id"/>
          </xsl:attribute>
        </xsl:when>
        <xsl:otherwise>
  ...
  <xsl:template match="note" priority="2" mode="stage2">
    <note>
      ...
      <xsl:if test="not(ancestor::chord)">
        <xsl:copy-of select="@artic"/>
        <xsl:apply-templates select="artic" mode="stage2"/>
      </xsl:if>
  ...
  <!-- <xsl:template match="note[not(parent::chord)]" priority="2" mode="stage2">
    <note>
      <xsl:copy-of
        select="@*[not(name()='meiform') and not(name()='beam')
                           and not(name()='tstamp.ges')]"/>
  ...
  <!-- On notes with artic sub-elements, copy articulation to parent chord.
       Don't copy tstamp.ges attribute.
  <xsl:template match="note[artic]" priority="2" mode="stage2">
    <note>
      <xsl:copy-of
        select="@*[not(name()='meiform') and not(name()='beam')
                           and not(name()='tstamp.ges')]"/>
      ...
      <xsl:when
        ...
        <xsl:variable name="measuredur">
          <xsl:choose>
            ...
            <xsl:when test="$meter.unit &gt; 4">
              <xsl:value-of select="($meter.count div ($meter.unit div 4)) * $ppq"/>
            </xsl:when>
          </xsl:choose>
        </xsl:variable>
