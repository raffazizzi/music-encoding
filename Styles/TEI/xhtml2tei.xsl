...
<xsl:variable name="author">
 <xsl:choose>
   ...
   <xsl:when test="h:body/h:address">
    ...
    <xsl:choose>
      <xsl:when test="contains($add,'Author: ')"><xsl:value-of select="substring-after($add,'Author: ')"/></xsl:when>
...
<xsl:variable name="date">
 <xsl:choose>
   <xsl:when test="h:body/h:address">
    ...
    <xsl:choose>
      <xsl:when test="contains($add,'Date: ')"><xsl:value-of select="substring-after($add,'Date: ')"/></xsl:when>
      <xsl:when test="contains($add,'Last updated: ')"><xsl:value-of select="substring-after($add,'Last updated: ')"/></xsl:when>
      <xsl:when test="contains($add,'Last Updated: ')"><xsl:value-of select="substring-after($add,'Last Updated: ')"/></xsl:when>
      <xsl:otherwise><xsl:value-of select="$add"/></xsl:otherwise>
    </xsl:choose>
   </xsl:when>
...
<TEI.2>
  <teiHeader>
	...
	<address><email>advisory@oucs.ox.ac.uk</email></address>
      </publicationStmt>
         <sourceDesc>
...
created by converting an HTML file to TEI XML on TODAYSDATE.</p>
      ...
      <revisionDesc>
         <change>
	    <date>$Date: 2008-04-22 18:48:12 +0100 (Tue, 22 Apr 2008) $</date>
            <respStmt><name>$Author: rahtz $</name></respStmt>
	    <item>$Revision: 4544 $</item>
         </change>
      </revisionDesc>
  </teiHeader>
  <text>
...
<xsl:template match="h:span[@class]"><!-- just continue -->
  <hi rend="{@class}"><xsl:apply-templates/></hi>
</xsl:template>
<!-- links -->
...
<xsl:choose>
 ...
 <xsl:otherwise>
   ...
   <xsl:otherwise>
     <xref url="{@href}"><xsl:value-of select="$content"/></xref>
   </xsl:otherwise>
   </xsl:choose>
 </xsl:otherwise>
</xsl:choose>
...
<xsl:template match="h:td">
 ...
 <xsl:if test="@valign">
  ...
  <xsl:attribute name="align"><xsl:value-of select="child::h:p/@align"/></xsl:attribute>
  </xsl:if>
   <xsl:apply-templates/>
 </cell>
</xsl:template>
...
<xsl:choose>
  <xsl:when test="$until &lt; $i">
    <xsl:text>-</xsl:text><xsl:value-of select="$stepsize"/>
  </xsl:when>
  <xsl:otherwise>
