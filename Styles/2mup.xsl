...
<!-- ************************************************************** -->
<!--
NAME:     File      = 2mup.xsl (version 1.0)
          Vers Date = 2009/07/27

NOTICE:   Copyright (c) 2007 Perry Roland and the Rector and
...
<xsl:stylesheet version="2.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
  ...
  <xsl:template match="meihead">
    ...
    <xsl:if test="filedesc/titlestmt/respstmt/agent">
      <xsl:text>// </xsl:text>
  ...
  <xsl:template match="scoredef">
    ...
    <xsl:if test="@key.sig">
      ...
      <xsl:choose>
        ...
        <xsl:otherwise>
          <xsl:value-of select="replace(replace(@key.sig,'f','&#x0026;'),'s','#')"/>
        </xsl:otherwise>
      </xsl:choose>
      <xsl:value-of select="$nl"/>
    </xsl:if>
  ...
  <xsl:template name="mupstrings2">
    ...
    <xsl:choose>
      <xsl:when test="count($list) gt 0">
        <xsl:variable name="pname" select="replace($list[position()=1],'[0-9]','')"/>
      ...
      <xsl:variable name="ppq">
        <xsl:choose>
          <xsl:when test="preceding::staffdef[@n='1'][@midi.div]">
            <xsl:value-of select="preceding::staffdef[@n='1'][@midi.div][1]/@midi.div"/>
          </xsl:when>
          <xsl:when test="descendant::staffdef[@n='1'][@midi.div]">
            <xsl:value-of select="descendant::staffdef[@n='1'][@midi.div][1]/@midi.div"/>
          </xsl:when>
          <xsl:when test="descendant::note[@dur='4' and @dur.ges]">
            <xsl:value-of select="following::note[@dur='4' and @dur.ges][1]/@dur.ges"/>
          </xsl:when>
          <xsl:otherwise>
  ...
  <xsl:template match="measure">
    ...
    <xsl:for-each select="staff[descendant::verse]">
      ...
      <xsl:variable name="lyricstring">
        <xsl:variable name="versecount">
          <xsl:value-of select="count(layer[descendant::verse]/*[verse][1]/verse)"/>
        </xsl:variable>
        <xsl:call-template name="verse">
    ...
    <xsl:choose>
      ...
      <xsl:when test="@id=parent::ending/measure[last()]/@id">
  ...
  <xsl:template match="chord">
    ...
    <xsl:apply-templates select="note[not(@staff)] | note[@staff=ancestor::staff/@n]"/>
    <xsl:if test="note[@staff != ancestor::staff/@n]">
      <xsl:text> with </xsl:text>
      <xsl:apply-templates select="note[@staff != ancestor::staff/@n]"/>
      <xsl:choose>
        <xsl:when test="note[@staff != ancestor::staff/@n][1] &gt; ancestor::staff/@n">
  ...
  <xsl:template name="tupletstart">
    <xsl:if test="starts-with(@tuplet, 'i')">
      <xsl:text>{</xsl:text>
    </xsl:if>
  </xsl:template>
  ...
  <xsl:template name="artic">
    <xsl:variable name="artics">
      ...
      <xsl:for-each select="artic[not(@source) or contains(string(@source),$source)]">
        <xsl:choose>
