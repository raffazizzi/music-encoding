...
<xsl:stylesheet version="2.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
  ...
  <xsl:template match="scoredef">
    ...
    <xsl:value-of select="$nl"/>
    <xsl:apply-templates select="staffgrp|staffdef"/>
    <!--  mode="def" -->
  ...
  <xsl:template match="staffgrp">
    <!--  mode="def" -->
    <xsl:apply-templates select="staffdef|staffgrp"/>
    <!--  mode="def" -->
  </xsl:template>
  ...
  <xsl:template match="measure">
    ...
    <xsl:if test="count(preceding::measure) = 0 and @left='rptstart'">
    ...
    <xsl:for-each select="staff[descendant::verse]">
      <xsl:for-each select="layer[descendant::verse]">
        <xsl:for-each-group select="descendant::verse" group-by="@n">
          ...
          <xsl:text>: </xsl:text>
          <xsl:for-each select="ancestor::layer">
            <xsl:for-each
              select="descendant::note[not(@grace)]|descendant::chord[not(@grace)]|
          descendant::rest|descendant::space">
            ...
            <xsl:variable name="lyricstring">
              <xsl:for-each select="descendant::syl">
                <xsl:if test="ancestor::verse/@n=$versenum">
                  <xsl:value-of select="."/>
                  <xsl:choose>
                    <xsl:when test="@con='d'">
                      <xsl:text>-</xsl:text>
    ...
    <xsl:if test="count(following::measure)=0">
      <xsl:apply-templates select="preceding::phrase|preceding::slur"
        mode="special"/>
      <xsl:apply-templates select="preceding::tie" mode="special"/>
    </xsl:if>
        ...
        <xsl:choose>
          ...
          <xsl:when test="$starttype != '' and $endtype = ''">
            ...
            <xsl:variable name="thislayer">
              <xsl:value-of
                select="generate-id(preceding::note[@xml:id=$thisend]/ancestor::layer)"
              />
            </xsl:variable>
            <xsl:value-of
  ...
  <xsl:template match="tie" mode="special">
  ...
  <xsl:template match="note">
    ...
    <xsl:choose>
      ...
      <xsl:when test="@pname">
        <xsl:if test="@staff and not(parent::chord)">
          <xsl:text> with </xsl:text>
        </xsl:if>
  ...
  <xsl:template name="tupletend">
    <xsl:if test="starts-with(@tuplet, 't') and not(ancestor::tuplet)">
      ...
      <xsl:choose>
        <xsl:when test="following::tupletspan[@endid=$id]/@num.place">
          <xsl:text> </xsl:text>
          <xsl:value-of select="following::tupletspan[@endid=$id][1]/@num.place"
          />
        </xsl:when>
      </xsl:choose>
