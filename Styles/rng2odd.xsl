                                    ...
                                    <xsl:for-each select="ancestor::grammar//define[@name=$this-mdl]">
                                        ...
                                        <xsl:choose>
                                            ...
                                            <xsl:when test="zeroOrMore and not(zeroOrMore/*[not(self::ref)]) and not(optional)">
                                                <xsl:value-of select="@name"/><xsl:text>_sequenceOptionalRepeatable</xsl:text>
                                            </xsl:when>
                                            <!-- If it doens't fit in any of the previous cases, it's a macro -->
                                            <xsl:otherwise>
                                                <xsl:text>macro.</xsl:text><xsl:value-of select="substring-after(@name, 'model.')"/>
                                            </xsl:otherwise>
                                        </xsl:choose>
                                    </xsl:for-each>
                                </xsl:attribute>
    ...
    <xsl:template match="/">
        ...
        <TEI>
            ...
            <teiHeader>
                ...
                <revisionDesc>
                    <change who="#RV" when="{current-date()}">
                        Converted RNG Datatypes Declarations into ODD Datatype Macros according to TEI guidelines. 
                        <ref target="http://www.tei-c.org/release/doc/tei-p5-doc/html/ST.html#DTYPES">Chapter 1, §1.4.2</ref>.
                    </change>
                    <change who="#RV" when="{current-date()}">
                        Converted RNG Attribute Classes Declarations into ODD Attribute Classes according to TEI guidelines. 
                        <ref target="http://www.tei-c.org/release/doc/tei-p5-doc/html/ST.html#STECAT">Chapter 1, §1.3.1</ref>.
                    </change>
                    <change who="#RV" when="{current-date()}">
                        Converted RNG Model Classes Declarations into ODD Model Classes according to TEI guidelines. 
                        <ref target="http://www.tei-c.org/release/doc/tei-p5-doc/en/html/ST.html#STECCM">Chapter 1, §1.3.2</ref>.
                        See also <ref target="http://www.tei-c.org/release/doc/tei-p5-doc/en/html/TD.html#TDCLA">Chapter 22, §22.4.6</ref> for references to models.
                    </change>
                    <change who="#RV" when="{current-date()}">
                        Converted RNG Model Classes Declarations into ODD Macro Classes (when appropriate) according to TEI guidelines. 
                        <ref target="http://www.tei-c.org/release/doc/tei-p5-doc/en/html/TD.html#TDENT">Chapter 22, §22.4.7</ref>.
                    </change>
                    <change who="#RV" when="{current-date()}">
                        Converted RNG Element Declarations into ODD Element Declarations according to TEI guidelines. 
                        <ref target="http://www.tei-c.org/release/doc/tei-p5-doc/en/html/TD.html#TDcrystals">Chapter 22, §22.3</ref>.
                        See also <ref target="http://www.tei-c.org/release/doc/tei-p5-doc/en/html/TD.html#TDCLA">Chapter 22, §22.4.6</ref> for references to models.
                    </change>
                </revisionDesc>
            </teiHeader>
        ...
        <xsl:choose>
            ...
            <xsl:when test="(ref)
                ...
                <classSpec type="model" ident="{@name}">
                    ...
                    -->
                    <classes>
                        <xsl:for-each select="ancestor::grammar//define[starts-with(@name, 'model.')]//ref[@name=$model-name]">
            ...
            <xsl:otherwise>
                <macroSpec type="pe" ident="macro.{substring-after(@name, 'model.')}">
                    ...
                    <content>
                        <xsl:for-each
                            select="*[not(self::a:documentation)]">
                            <xsl:call-template name="make-namespace-node">
                                <xsl:with-param name="handle-models" select="true()"/>
                                <xsl:with-param name="recursive" select="true()"/>
                            </xsl:call-template>
        ...
        <elementSpec ident="{@name}">
            ...
            <xsl:if test="ancestor::grammar//define[@name=concat('attlist.',$element-name)]/ref">
                <classes>
                    ...
                    -->
                    <xsl:for-each select="ancestor::grammar//define[starts-with(@name, 'model.')]//ref[@name=$element-name]">
                        <memberOf key="{ancestor::define/@name}"/>
                    </xsl:for-each>
                </classes>
            </xsl:if>
