...
<!-- ************************************************************** -->
<!--
NAME:     File      = 2mei.xsl (version 2.2.3)
          Vers Date = 2010/04/23

NOTICE:   Copyright (c) 2010 Perry Roland and the Rector and
...
CHANGES:  (v. 1.1)
          ...
          (v. 2.2.3)
          1. Generates file conformant with modularized MEI 1.9b (modularized)
          2. revised handling of vertical offset on rests          
                                                                    -->
<!-- ************************************************************** -->
...
following-sibling::measure[1][attributes[not(preceding-sibling::note)]] -->
  ...
  <xsl:template name="restvo2">
    ...
    <!-- Debugging code:
    <xsl:if test="@xml:id='d1e1383'">
      <xsl:message>THISSTAFF: <xsl:value-of select="$thisstaff"/></xsl:message>
      <xsl:message>WHERECLEF: <xsl:value-of select="$whereclef"/></xsl:message>
      <xsl:message>CLEFSHAPE: <xsl:value-of select="$clefshape"/></xsl:message>
      <xsl:message>CLEFLINE: <xsl:value-of select="$clefline"/></xsl:message>
      <xsl:message>DISPLAY-STEP: <xsl:value-of select="$display-step"
        /></xsl:message>
      <xsl:message>DISPLAY-OCTAVE: <xsl:value-of select="$display-octave"
        /></xsl:message>
    </xsl:if>
    -->
    ...
    <xsl:for-each select="notations/articulations/*">
      <artic>
        <xsl:choose>
          ...
          <xsl:when test="name()='detached-legato'">
            <xsl:attribute name="artic">ten_stacc</xsl:attribute>
            <xsl:if test="@placement != ''">
              <xsl:attribute name="place">
                <xsl:value-of select="@placement"/>
              </xsl:attribute>
      ...
      <xsl:when
        ...
        <xsl:choose>
          <xsl:when test="@dur.ges=$measuredur">
            ...
            <!-- Some debugging code:
            <xsl:message>THISSTAFF: <xsl:value-of select="$thisstaff"
              /></xsl:message>
            <xsl:message>PPQ: <xsl:value-of select="$ppq"/></xsl:message>
            <xsl:message>METER.UNIT: <xsl:value-of select="$meter.unit"
              /></xsl:message>
            <xsl:message>METER.COUNT: <xsl:value-of select="$meter.count"
              /></xsl:message>
            <xsl:message>MEASUREDUR: <xsl:value-of select="$measuredur"
              /></xsl:message>
